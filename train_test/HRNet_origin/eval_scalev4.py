...
def valid(model, valloader, input_size, num_samples, gpus):
    ...
    with torch.no_grad():
        for index, batch in enumerate(valloader):
            ...
            if index % 10 == 0:
                print('%d  processd' % (index * num_images),input1.size(),input2.size())
            ...
            if gpus > 1:
                for output1,output2 in zip(outputs1,outputs2):
                    nums = len(output1)
                    parsing = m*interp(output1).data.cpu().numpy() + n*interp(output2).data.cpu().numpy()#+ interp(output2).data.cpu().numpy())
